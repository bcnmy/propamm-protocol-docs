...
interface IMMProvider {
    ...
    // The fill. Pull amountIn from the caller, deliver at least amountOut to receiver.
...
3. Deliver `amountOut` of `tokenOut` from inventory to `receiver` and return the amount. `amountOut` is the exact order-book total the executor swept across your signed levels, so a basic implementation delivers it as-is; `anchorPrice` (the volume-weighted average of the same sweep, 1e18-scaled as tokenOut per tokenIn) is there for implementations that shape a venue order from a price.

The settlement contract imposes no cap of its own: the taker's `minAmountOut` floor is enforced at settlement, so delivering less than promised reverts the fill rather than settling worse. Return the matching value from `previewSwap` so quotes match execution.

You stream a price ladder (section 2), and for each fill the executor sweeps your levels from that ladder's lifetime fill cursor - an order overlapping levels blends across them, each slice at its tranche's price - and hands you the exact swept total as `amountOut`. Your contract never sees the ladder, only the resolved output and price for its fill.
...
function executeSwap(
    ...
    delivered = amountOut;   // the exact swept total; venue-backed makers source it here
...
How levels are consumed: `size` is cumulative depth, so a ladder of `[(1e18, pA), (5e18, pB)]` means the first 1 tokenIn fills at `pA` and the next 4 at `pB`. The executor tracks how much has ever filled against each signed ladder and sweeps each fill across your levels from that lifetime cursor - an order overlapping levels blends across them, each consumed slice at its tranche's price (your provider receives the exact swept output to deliver); total volume against one ladder can never exceed its top level's size, once, ever, and no supersede or re-commit can re-arm a consumed level.
...
- A fresher committed nonce supersedes and cancels the one before it, on the spot. A delayed update can never overwrite a fresher one: an older nonce is ignored while the newer one lives, and is accepted again only if the newer one has expired while the older is still inside its own signed validity (this is what lets short ticks and longer quotes share one nonce sequence).
- The expiry is the freshness bound, and it is yours per message.
- Publish at a steady cadence. A fresher stream serves more flow.

### Your price commits inside the settlement transaction

Your price commits to the chain inside the same transaction that settles the fill, before the fill. You sign and stream; the submitter does the on-chain commit. If a fresher ladder of yours is already committed and live when a transaction lands, its carried commit is a no-op and the fill settles at your freshest committed price instead - an in-flight trade never settles at a price you have already replaced.
...
| | |
|---|---|
| REST | `https://propamm.biconomy.io/v1` |
| WebSocket | `wss://propamm.biconomy.io` (same host, any path) |
| Chains | Base mainnet (8453), BNB Smart Chain (56) |
| Testing | Base Sepolia (84532, mintable test tokens) is served by the staging environment: `https://propamm-staging.biconomy.io/v1`, `wss://propamm-staging.biconomy.io` |

Same protocol, same message shapes on both; a client validated against Sepolia moves to
production by changing the endpoint and the token addresses.
...
|---|---|
| Base Sepolia (84532) | `0x000000Ab52Bdb44411777412fe938776a766cc6a` |
| Base mainnet (8453) | `0x000000Ab52Bdb44411777412fe938776a766cc6a` |
| BNB Smart Chain (56) | `0x000000Ab52Bdb44411777412fe938776a766cc6a` |
...
- **Base mainnet (8453)** and **BNB Smart Chain (56)** contracts are deployed and source-verified at the identical addresses. Mainnet onboarding is capped pending the external audit: start with small inventory, tight top-level sizes, and short expiries.
...
For reference only, since as a maker you need nothing but the executor above: PropAMMSettlement (the entrypoint aggregators call) `0x0000004c86FBE08cC6A39c6D4C87b59F71dE31bc`, Create2InitFactory `0xCBDBc177096B1fCD32242c346802a61346e6031C`.
...
const ENDPOINT = "wss://propamm-staging.biconomy.io"; // Sepolia testing; production: wss://propamm.biconomy.io
const CHAIN_ID = 84532;
const EXECUTOR = "0x000000Ab52Bdb44411777412fe938776a766cc6a"; // identical on every chain
...
| Suggested `expiresAt` | yours per message: now + 2 to 10 s for hot-flow ticks, now + 15 to 60 s for quotes that ride aggregator pipelines; expired updates are rejected |
...
sequenceDiagram
    ...
    P->>S: heartbeat commits your freshest board onchain
    AGG->>S: reads your board in one eth_call, like any pool
    AGG->>S: submits its own transaction, pays its own gas
    Note over S: fills against your freshest committed board
    S->>MM: executeSwap(tokenIn, tokenOut, amountIn, anchorPrice, amountOut, receiver)
...
## Sizing your ladders for the execution lanes

Three knobs in the ladders you sign map directly onto how integrators consume you:

- **Dual TTL, one stream.** Interleave fast tight quotes (5-15s TTL) with occasional
  longer-lived, slightly wider ones (30-60s TTL) on the SAME monotonic nonce sequence. Fast
  quotes serve the hosted flow at your tightest price; the longer-lived ones keep your onchain
  board serviceable between heartbeats and can serve RFQ-style pipelines that carry your
  signed quote in their own transaction - the window is priced by you, since you set the TTL.
  A paused stream is covered by dead-air revival.
  Send the long quote BEFORE the same-cycle fast one so the tick keeps the freshest nonce.
- **Top-level size = your per-version exposure cap.** Depth budgets are once-ever per signed
  version: across every route and every retry, a version can never fill more than its top
  level. Size it as the maximum you are willing to trade at that version's prices.

## The onchain lane (zero maker work)

We deploy a `PropAMMPool` fronting your board and run the heartbeat that keeps it live:
routers read your prices in one eth_call and fill you with a deterministic onchain swap - no
API in their hot path, no additional signing or infrastructure on your side. This is how
aggregators consume the venue. Your inventory custody, TTLs and budgets are unchanged; the pool can only execute
through the same permissionless door as every other route.
