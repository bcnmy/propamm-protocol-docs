...
PropAMM lets market makers quote onchain from an offchain signed stream. A maker signs a board for each direction it quotes: either a price ladder (cumulative sizes with absolute prices) or an offset ladder (cumulative sizes with discounts below a reference price) plus anchors (the reference price itself, signed as often as every block). It may also sign one set of board controls, the risk limits the executor applies to every fill of that board. Anyone commits those messages to `PropAMMExecutor`, which stores the whole board and enforces it on every fill. Two lanes consume the boards: aggregator routers fill `PropAMMVenue` like any pool, and the network settles end users' signed intents through `PropAMMHostedSettlement`.

One guarantee for takers: the receiver's real balance grows by at least the floor, or the trade reverts. One guarantee for makers: inventory only ever fills at a price the maker signed, within the depth, the expiry and the limits the maker signed.

## Two lanes over one executor

```mermaid
flowchart TD
    MM["Market maker<br/>pricing engine, one signing key"] -->|"price ladders, or offset ladders plus anchors, plus board controls"| NET["Network service (offchain)<br/>stream validation, keeper, relayers"]
    NET -->|"updatePrices / updateOffsets / updateAnchors / updateControls"| EXE
...
|---|---|---|
| `PropAMMExecutor` | Stores one board per maker and direction, verifies commits, prices, meters and limits every fill | No |
...
| Board controls | honoured: a capped maker contributes only what it may still fill this block, and the merge routes the rest to others | honoured: a fill above the block's remaining cap reverts and the intent fails alone |
...
Commits are batched (`ladders[]`, `sigs[]`) and permissionless. Inside a batch a message whose nonce is not fresher than the board's is skipped silently, before validation or signature checking; a fresher message that is malformed, expired or badly signed is skipped and reported as `CommitRejected(mm, tokenIn, tokenOut, kind, reason)`; the rest of the batch still commits. On the Base and BNB executors such a message reverts the whole batch instead. The depth nonce is shared by price and offset ladders for the same board and must strictly increase; the anchor nonce is independent and must strictly increase too. A depth commit rebinds the provider, replaces every level, sets the mode and drift, and resets the meter. An anchor commit writes one storage word and leaves the meter alone. An anchor committed while the board is a price ladder is stored and unused until the maker commits an offset ladder.
...
| Field | Effect |
|---|---|
| `blockCap` | The most `tokenIn` any single block may fill from this board, across both lanes and every caller. Zero disables it. |
...
| `controls(address mm, address tokenIn, address tokenOut) returns (uint256 blockCap, uint256 widenPpmPerSqrtSecond, uint256 premiumPpm, uint256 premiumBlocks, uint256 nonce, uint256 filledThisBlock, uint256 lastCommitBlock, uint256 committedAt)` | view | The board's controls and what this block has already filled |
| `providerOf(address mm, address tokenIn, address tokenOut) returns (address)` | view | The provider bound at the last depth commit |
| `hashLadder(PriceLadder)`, `hashOffsetLadder(OffsetLadder)`, `hashAnchor(Anchor)`, `hashControls(BoardControls)` | pure | EIP-712 struct hashes |
| `ladderDigest(PriceLadder)`, `offsetLadderDigest(OffsetLadder)`, `anchorDigest(Anchor)`, `controlsDigest(BoardControls)` | view | Full digests, what a maker's key signs |

`mode` in `board()` is `0` for a board that has never been committed, `1` for a price ladder and `2` for an offset ladder.

Events: `LadderCommitted`, `OffsetsCommitted`, `AnchorCommitted`, `ControlsCommitted` when a message is accepted, and `MMFillExecuted(mmProvider, mmSigner, receiver, tokenIn, tokenOut, amountIn, amountOut, avgPrice, filledAfter)` per fill, where `filledAfter` is the meter after the fill.
...
sequenceDiagram
    ...
    M->>N: signed Anchor per direction, as often as every block on offset boards
    M->>N: signed BoardControls per board, rarely and optional
    N->>E: updateOffsets or updatePrices, batched
    N->>E: updateAnchors, both directions in one call
    N->>E: updateControls when the limits change
...
sequenceDiagram
    ...
    A->>V: eth_call quote for tokenIn, tokenOut, amountIn
    V->>E: board for each registered maker
    A->>V: transfer amountIn, then swap with minAmountOut, recipient, deadline
    V->>E: transfer each maker's share, then fill it
    E->>P: executeSwap, tokenOut to the recipient
...
sequenceDiagram
    ...
    U->>N: request a quote
    N-->>U: Intent to sign, one EIP-712 signature with a Permit2 witness
    U->>N: signed Intent
    N->>S: settle batch
    S->>E: commits, when the batch carries them
    S->>S: pull tokenIn through Permit2, snapshot receiver
    S->>E: step transfer amountIn, then step fill for mm, tokenIn, tokenOut, amountIn, receiver
